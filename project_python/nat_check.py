        ...
        while not recieved:
            ...
            try:
                ...
                if count > 0:
                    ...
                    retVal['Resp'] = False
                    return retVal
        msgtype = str(binascii.b2a_hex(buf[0:2]), encoding='utf-8')
...
def get_nat_type(s, source_ip, source_port, stun_host=None, stun_port=3478, stun_servers_list=STUN_SERVERS):
    _initialize()
    port = stun_port
    log.debug("Do Test1")
    print("Do Test1")
    ...
    if stun_host:
        ...
        for stun_host in stun_servers_list:
            ...
            print('Trying STUN host: %s' % stun_host)
            ret = stun_test(s, stun_host, port, source_ip, source_port)
            resp = ret['Resp']
            if resp:
                break
...
def get_ip_info2(source_ip="0.0.0.0", source_port=54320, stun_host=None,
                stun_port=3478):
    socket.setdefaulttimeout(2)
    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    ...
    nat_type, nat = get_nat_type(s, source_ip, source_port,
                                 stun_host=stun_host, stun_port=stun_port)
    if nat_type == ChangedAddressError:
        nat_type, nat = get_nat_type(s, source_ip, source_port,
                                 stun_host=stun_host, stun_port=stun_port, stun_servers_list=SELF_SERVERS)
    external_ip = nat['ExternalIP']
    external_port = nat['ExternalPort']
    s.close()
    return nat_type
    # return NET_TYPE_MAP[nat_type]


nat_type = get_ip_info2(source_port=random.randint(40000,50000))
print("nat type: %s %s" % (str(NET_TYPE_MAP[nat_type]), nat_type))